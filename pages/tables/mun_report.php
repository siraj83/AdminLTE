...
<head>
    ...
    <title>منظومة المكبات | تقرير عن إستبيان البلديات الشريكة</title>
        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row" style="direction: rtl;text-align: center;display: block;">
                    <div class="col-sm-6" style="text-align: left;">
                        <h1>تقرير عن إستبيان البلديات الشريكة</h1>
                    </div>
                    <!-- <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">جداول البيانات</li>
                ...
                <form action="../charts/municipalities.php">
                    <div class="col">
            ...
            <div class="container-fluid">
                <div class="row" style="direction: rtl;">
                    <div class="col-12">
                            ...
                            <div class="card-body">
                                <table id="example2" name="example2" class="table table-bordered table-hover"
                                    ...
                                    <thead>
                                        <tr>
                                            <th>الاسم</th>
                                            <th>من يتولى عملية جمع المخلفات الصلبة</th>
                                            <th>تقدم الخدمة عن طريق</th>
                                            <th>من يتولى عملية النقل المخلفات الصلبة</th>
                                            <th>تقدم الخدمة عن طريق</th>
                                            <th>محطة ترحيل</th>
                                            <th>محطة ترحيل مجهزة</th>
                                            <th>يقتصر دورها على</th>
                                            <th>المعوقات</th>
                                            <th>مستوى الخدمة الحالي</th>
                      ...
                      $sql = "SELECT id,`Mname`,`who_handle_coll`,`service_coll_by`,`who_handle_mov`,`service_mov_by`,`transfer_station`,`equi_station`,`mun_role`,`reasons`,`service_level` from municipalities where who_handle_coll <> ''";
                      $result = $conn->query($sql);
                                         if ($result->num_rows > 0) {
                        while ($row = $result-> fetch_assoc()){
                         echo '<tr data-value="2">' ;
                          echo  "<td>" . $row["Mname"] . "</td><td>" . $row["who_handle_coll"]."</td><td>" .$row["service_coll_by"]."</td><td>" .$row["who_handle_mov"]."</td><td>" .$row["service_mov_by"]. "</td><td>" .$row["transfer_station"]. "</td><td>" . $row["equi_station"] . "</td><td>" . $row["mun_role"] . "</td><td>". $row["reasons"] . "</td><td>". $row["service_level"] . "</td><td>"?>
